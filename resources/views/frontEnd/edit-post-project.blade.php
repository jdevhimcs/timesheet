    ...
    <form class="form-horizontal" role="form" action="{{ route('update-project', [$item->id]) }}" method="POST" enctype="multipart/form-data">
     ...
     <div class="card">
        <div class="card-header">
          <h5 class="card-title">Edit Project Posting</h5>
        </div>
      <div class="card-body">
  ...
  <div class="form-group col-md-6">
   ...
   <select class="form-control no-radius"  name="professional_quality[]" multiple="multiple" required="required" data-placeholder="The 3 of 10 best qualities of the professiona">
      @foreach($qualities as $key=>$value)
        <option value="{{$value->id}}" @if(in_array($value->id, explode(',', $item->professional_quality))) selected @endif>{{$value->quality}}</option>
      @endforeach
  ...
  <div class="form-group col-md-6">
   ...
   <select class="form-control no-radius" disabled name="status" required="required"  data-placeholder="status">
     <option value="">Please Select</option>
  ...
  <div class="form-group col-md-12">
   ...
   <input type="file" class="form-control" name="attachment" placeholder="">
          ...
          @if(!empty($item->attachment))
            <br>
            <a href="{{ asset('/images/projectimage/'.$item->attachment) }}" target="__blank">Click here</a> to view attachmnet.
          @endif
  </div>
